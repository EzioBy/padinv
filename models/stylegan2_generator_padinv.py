# python3.7
"""Contains the implementation of generator described in PadInv.

Based on file `models/stylegan2_generator.py`, the generator in this file allows
replacing the zero paddings in convolutional layers with customized
coefficients. Such an replacement is primarily used for GAN inversion with
padding space (i.e., the PadInv paper).
...
__all__ = ['StyleGAN2GeneratorPadInv']
...
def _center_crop(tensor, target_size):
    """Crops a `tensor` into `target_size`.

    Args:
        tensor: The tensor to be cropped, assumed in shape of
            [batch_size, channels, height, width].
        target_size: The spatial size of returned tensor, can be an `int`
            (square), a list, or a tuple with (target_height, target_width).

    Returns:
        A tensor with target spatial shape center cropped from `tensor`.
    """
    height, width = tensor.shape[2], tensor.shape[3]
    target_height, target_width = format_image_size(target_size)
    if height == target_height and width == target_width:
        return tensor

    y = (height - target_height) // 2
    x = (width - target_width) // 2
    return tensor[:, :, y:y + target_height, x:x + target_width]

# pylint: disable=missing-function-docstring

class StyleGAN2GeneratorPadInv(nn.Module):
    ...
    def forward(self,
                ...
                trunc_psi=None,
                trunc_layers=None,
                padding_maps=None,  # A list of per-layer adding coefficients.
                noise_mode='const',
...
class SynthesisNetwork(nn.Module):
    ...
    def forward(self,
                wp,
                padding_maps=None,  # A list of per-layer padding coefficients.
                noise_mode='const',
                fused_modulate=False,
                fp16_res=None,
                impl='cuda'):
        ...
        # Ensure there is a padding map for each layer. Here, layer includes
        # the initial constant yet excludes the final ToRGB layer.
        if padding_maps is None:
            padding_maps = [None] * self.num_layers
        assert isinstance(padding_maps, (list, tuple))
        padding_maps = list(padding_maps)
        if len(padding_maps) < self.num_layers:
            padding_maps.extend([None] * (self.num_layers - len(padding_maps)))
        assert len(padding_maps) >= self.num_layers

        # Replace the constant input with the first padding map if needed.
        padding_init = padding_maps[0]
        if padding_init is not None:
            x = _center_crop(padding_init, self.init_res)
        else:
            if self.const_input:
        ...
        if self.architecture == 'origin':
            for layer_idx in range(self.num_layers - 1):
                ...
                x, style = layer(x,
                                 wp[:, layer_idx],
                                 padding_map=padding_maps[layer_idx + 1],
                                 noise_mode=noise_mode,
                                 fused_modulate=fused_modulate,
                                 impl=impl)
            ...
            for layer_idx in range(1, self.num_layers - 1, 2):
                ...
                x, style = layer(x,
                                 wp[:, layer_idx + 1],
                                 padding_map=padding_maps[layer_idx + 2],
...
class ModulateConvLayer(nn.Module):
    ...
    def forward(self,
        ...
        if self.scale_factor == 1:  # Native convolution without upsampling.
            up = 1
            padding = self.kernel_size // 2
            if padding_map is not None:
                padding_map = _center_crop(padding_map, self.resolution + 2)
                # Padding x is equivalent to copying x to the center of the
                # padding map.
                padding_map[:, :, 1:-1, 1:-1] = x
                x = conv2d_gradfix.conv2d(padding_map,
                                          w,
                                          stride=1,
                                          padding=0,
                                          groups=groups,
            ...
            # When kernel size != 1, use stride convolution for upsampling.
                ...
                # For odd layers, the padding map will be padded to the feature
                # map after the transposed conv layer.
                if padding_map is not None:
                    padding = (0, 0, 0, 0)
                    padding_map[:, :, 1:-1, 1:-1] = x
                    x = upfirdn2d.upfirdn2d(padding_map,
                                            f,
                                            up=1,
                                            padding=padding,
                                            gain=up ** 2,
                                            impl=impl)
                # If no padding map is passed, just use 0 to pad.
                else:
                    padding = (px0 + pxt, px1 + pxt, py0 + pyt, py1 + pyt)
                    x = upfirdn2d.upfirdn2d(
                        x, f, up=1, padding=padding, gain=up ** 2, impl=impl)
