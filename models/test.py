...
def test_arcface():
    """Test the ArcFace model."""
    print('===== Testing ArcFace Model =====')

    print('Build test model.')
    model = build_model('ArcFaceModel')

    print('Test performance: ')
    for size in [64, 128, 256, 512, 1024]:
        img = torch.randint(0, 256, size=(_BATCH_SIZE, 3, size, size))
        img = img.to(torch.float32).cuda() / 127.5 - 1
        feat = model(img)
        assert feat.shape == (_BATCH_SIZE, 512)

        print(f'    Size {size}x{size}: feature norm '
              f'{feat.square().sum(dim=1).mean().item():.3e}.')
