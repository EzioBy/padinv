...
class BaseConfig(object):
    ...
    def get_options(cls):
        ...
        options['Logging and file system settings'].extend([
            ...
            cls.command_option(
                '--file_transmitter_type', type=str, default='local',
        ...
        options['Controller settings'].extend([
            ...
            cls.command_option(
                '--keep_ckpt_num', type=cls.int_type, default=20,
            ...
            cls.command_option(
                '--arnold_interval', type=cls.int_type, default=10000,
                help='How often to pull files from Arnold platform to HDFS. '
                     'Set as non-positive to disable.'),
            cls.command_option(
                '--keep_archive_num', type=cls.int_type, default=20,
                help='How many most recent Arnold archives to keep. Set as '
                     'non-positive to keep all archive files on HDFS.'),
            cls.command_option(
                '--hdfs_upload_path', type=str, default=None,
                help='HDFS path to upload archive files.'),
            cls.command_option(
                '--use_local_tb', type=cls.bool_type, default=False,
                help='Whether to use local TensorBoard on Arnold platform.')
        ])
    ...
    def get_recommended_options(cls):
        ...
        recommended_options = [
            ...
            'eval_at_start', 'arnold_interval', 'hdfs_upload_path',
            'keep_archive_num'
        ]
    ...
    def parse_options(self):
        ...
        arnold_interval = self.args.pop('arnold_interval')
        if arnold_interval > 0:
            self.config.controllers.update(
                ArnoldController=dict(
                    every_n_iters=arnold_interval,
                    first_iter=False,
                    keep_archive_num=self.args.pop('keep_archive_num'),
                    hdfs_upload_path=self.args.pop('hdfs_upload_path'),
                    use_local_tb=self.args.pop('use_local_tb')
                )
            )
